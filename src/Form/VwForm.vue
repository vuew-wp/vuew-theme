<template>
    <div class="uk-padding-large">
        ...
        <p v-text="status"></p>
        <form @submit.prevent="$emit('process-form', userData)" class="uk-grid uk-form-stacked" novalidate>

            <div v-for="field in formData.properties" :key="field.id" class="uk-width-1-1">

                <form-field-password v-if="'password' === field.type" :field="field"
                                     v-on:input="handleChange"/>

                <form-field-text-area v-else-if="'textarea' === field.type" :field="field"
                                      v-on:input="handleChange"/>

                <form-field v-else :field="field"
                            v-on:input="handleChange"/>
