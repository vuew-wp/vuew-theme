<template>
  <div :class="inputStateChangeClass" class="uk-margin-bottom">
    ...
    <div v-if="field.confirmInput">
      <label :for="'vw-form-field-' + field.id + '-confirm'"
        >Confirm Password</label
      >
      <input
        :id="'vw-form-field-' + field.id + '-confirm'"
        type="password"
        placeholder="Confirm password"
        class="uk-input"
        v-on:input="matchField($event.target.value, field)"
      />
    </div>
    <div v-if="field.strengthIndicator">
      <p v-html="(strength / 8) * 100 + '%'"></p>
      <div
        class="uk-width-1-1 uk-position-relative vw-password-strength-indicator"
      >
        <div
          :style="{ width: 100 - (strength / 8) * 100 + '%' }"
  ...
  methods: {
    ...
    validPassword(value) {
      ...
      for (let regex in this.passwordRegex) {
        if (
          !this.passwordRegex.hasOwnProperty(regex) ||
          'hasRepetition' === regex ||
          'hasSpace' === regex
        ) {
          continue;
      ...
      if (
        valLeng >= 1 &&
        valLeng < 9 &&
        this.passwordRegex.hasRepetition.test(value)
      ) {
        --finalScore;
      }
