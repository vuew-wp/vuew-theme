<template>
  <article
    :class="[
      'uk-width-1-' + columns + '@m',
      displayExcerpt ? 'post-vuew-content-open' : 'post-vuew-content-closed',
    ]"
    class="uk-width-1-1@s uk-margin-bottom post-vuew"
  >
    <div class="uk-position-relative uk-float-left uk-width-1-1">
      <attachment
        :src="post.featured_media.thumbnail"
        :type="'background'"
        class="post-vuew-image uk-position-cover"
      ></attachment>
      <div class="post-vuew-title uk-position-bottom">
        <router-link
          :to="setMenuItemParams(post)"
          class="uk-position-relative uk-float-left uk-width-1-1"
        >
          <h3 v-html="truncate(post.title, 4)" class="uk-h3"></h3>
        </router-link>
        <a
          @click="toggleExcerpt"
          class="post-vuew-excerpt-toggle uk-margin-top uk-position-bottom-center"
          v-html="require('../../svg/arrow-down.svg')"
        ></a>
      </div>
      <span
        v-text="post.date"
        class="post-vuew-date uk-position-top-left uk-background-primary uk-text-small"
      ></span>
    </div>
    <div class="post-vuew-excerpt">
      ...
      <p>
        <router-link
          :to="setMenuItemParams(post)"
          class="uk-button uk-width-1-1 uk-button-primary uk-button-small"
        >
          Read More
        </router-link>
      </p>
